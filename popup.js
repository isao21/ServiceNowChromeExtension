...
changeColor.addEventListener("click", async () => {
 ...
 chrome.scripting.executeScript({
  ...
  for (let i = 0; i <injectionResults.length; i ++){
    console.log("url is " + injectionResults[i] + "string " + JSON.stringify(injectionResults[i].result ))
  //  console.log("url is " + JSON.stringify(injectionResults))

  
      if (injectionResults[i].result != null){
        console.log("result is not null")
        injectionResults[i].result.forEach(element =>{
          chrome.tabs.create({
            url:element,
            active: false
...
*/
  ...
  if(window.frameElement.name ==='gsft_main'){
    console.log("main frame indentified")
  return firmLinks;
}
 }
